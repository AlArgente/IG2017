...
void _file_ply::read(vector<Point3d> &points, vector<Face> &faces) {
  string Token;
    ...
    if (Token == "format") {
      ...
      if (Token == "ascii") {
        ...
        if (Token == "vertex") {
          ...
          if (Token == "face") {
            get_token(Line_stream, Token);
            faces.resize(stoi(Token));
            // cout << "Tamaño de las caras: " << faces.size() << endl;
            do {
              get_new_line(File, Line_stream);
              get_token(Line_stream, Token);
            ...
            for (unsigned int i = 0; i < points.size(); i++) {
              ...
              get_token(Line_stream, Token);
              points[i].x = stof(Token);
              get_token(Line_stream, Token);
              points[i].y = stof(Token);
              get_token(Line_stream, Token);
              points[i].z = stof(Token);
              /*cout << "Los puntos leídos son: x = " << points[i].x
                   << " y = " << points[i].y << " z = " << points[i].z <<
                 endl;*/

              //              cout << " leido " << Vertices[i].x << " " <<
            ...
            for (unsigned int i = 0; i < faces.size(); i++) {
              ...
              if (stoi(Token) == 3) {
                get_token(Line_stream, Token);
                faces[i].v1 = stoi(Token);
                get_token(Line_stream, Token);
                faces[i].v2 = stoi(Token);
                get_token(Line_stream, Token);
                faces[i].v3 = stoi(Token);
              }
            }
          } else {
...
void _file_ply::close() { File.close(); }
